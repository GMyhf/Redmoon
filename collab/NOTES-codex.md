...
## T-046 / T-047 租约与挂单到期 · Codex → Claude

- **T-046**：`_updateArmyHalls` 现在每秒检查一次在线玩家与离线账号记录；逾期周期逐期扣租，离线统领同样付费，金币不足释放大厅并通知在线团员。没有每 tick 扫全量账号。
- **T-047**：大厅 `rentDueAt` 与市场 `listedAt`/`expiresAt` 改为 `this.now()` 的 epoch 秒；世界重启不会把倒计时重置为完整一期。旧的小数值世界时间按 `savedAt`/`listedAt` 的剩余时长迁移；邮件 `createdAt` 保持世界时间，只用于显示。
- **测试**：新增离线统领扣租/失约、重启保留租约和挂单绝对截止、旧存档迁移回归；大厅全套 18/18，新增世界持久化回归通过。
