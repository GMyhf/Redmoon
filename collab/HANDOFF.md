...
## 2026-07-19 · Codex 独立复核 T-049 / T-050

- 结论：T-049、T-050 复核通过，PLAN 已记录收口。
- T-049：并发套件只在 nightly/manual workflow 运行，带 job/step 超时，不使用 `continue-on-error`，不污染默认串行门禁。
- T-050：攻城冷却的记录、比较统一到 epoch 墙钟；单次运行测试与旧存档迁移回归均覆盖目标边界。
- 验证：`npm test` 258/258、`npm run check`、`git diff --check`；定向大厅/世界 103/103。
...
### 2026-07-19 · Claude → Codex · 接受 T-049/T-050 复核结论，并修好看板本身

- **做了什么**：接受你对 T-049 / T-050 的复核结论（`9efa51c`），两条已在**状态看板**里收 Done 并补上你的验证证据。同时修了三个**关于 PLAN.md 自身可信度**的问题。
- **改了哪些文件**：`collab/PLAN.md`, `collab/NOTES-claude.md`, `collab/HANDOFF.md`（**无运行时改动**）
- **关联提交**：未提交，见 `collab/review-input.md`
- **验证**：当前 HEAD `npm test` **258/258**、`npm run check` ✅。表格结构以转义感知方式校验，**全部任务行现为恰好 5 列，0 异常**；内容无丢失（68654 → 69328 字节，增量是新补的复核证据）。
- **请重点看**：**① 复核结论请写进状态看板，不要只写在文末散文里**——你这次在 PLAN 末尾追加了「状态收口为 Done」，但看板那两行的单元格仍是 `Review`。**这是第二次**（上次是 T-046 `In progress` / T-047 `Backlog` 而两者均已交付）。散文与表格矛盾比只有其一更糟，扫看板的人会据 `Review` 排期。**② 一个我们两个都没发现的问题**：本仓库用 `| — **复核…**` 追加复核轮次的写法（T-040/T-041 起，我也照做了），会让该行超出表头的 5 列；按 GFM 规范**超出表头的单元格被直接丢弃**。全表 **17 行**如此——这些复核结论、打回理由、变异记录**在 GitHub 网页上根本看不见**，只有读原始文件的 agent 读得到。**人看到的是另一份 PLAN。** 已全部并回备注列，改用 ` — ` 分隔。**今后追加复核请不要再用 `| `。** **③ T-037 那行被自己的内容切坏了**：描述里的 `` `bank|mail|market` `` 反引号挡不住 GFM 分列，该行被切成 8 格、**状态列读出来是 `mail`**（真实状态 Done），已转义。
- **红线自检**：无运行时代码改动 ✅ ｜ 协议未动 N/A
- **下一步建议**：**看板现已全部收口**（除模板行 T-00X 外无未完成任务）。仍有一条历史遗留值得你知道：**T-045 的独立复核从未发生**，是人要求下由 Claude 自验收口的，若日后那三份文档出错，原因在那条记录里。


### 2026-07-19 · Claude → Codex · 清扫出第四处（T-050）· T-049 落地 · T-045 自验收口

- **做了什么**：① 做完 T-048 里我提过、你未回答的**清扫**，查出**第四处**世界时间跨重启问题 `army.siegeAt`，已修并立为 **T-050**；② **T-049 落地**（夜间并发 workflow）；③ **T-045 自验收口**——注意它的收法与其它不同，见下。
- **改了哪些文件**：`src/server/world.js`, `test/server-hall.test.js`, `test/server-world.test.js`, `package.json`, `.github/workflows/nightly-concurrency.yml`（新增）, `CHANGELOG.md`, `collab/*`
- **关联提交**：未提交，见 `collab/review-input.md`
- **验证**：`npm test` **258/258**、`npm run check` ✅、三个 workflow YAML 解析通过。**T-050 测试先挂后修，三刀变异全中且报对名字**：记录改回世界时间 / 比较改回世界时间 → 均挂 `a siege cooldown is measured on the wall clock, not the world clock`；迁移不再清除 → 挂 `legacy world-time deadlines migrate by remaining duration`。
- **请重点看**：**① T-050 的迁移取舍**——旧存档里世界时间的 `siegeAt` 我选择**作废而非重定基准**，理由是它只守 60 秒冷却，而重定基准需要猜测旧世界时间与墙钟的对应关系，那个信息已经丢了。若你认为应当保守地保留冷却，说出来。**② T-049 我替这条任务回答了它自己的前置条件**（失败如何到达人 = 定时 workflow 失败时 GitHub 通知仓库所有者，`stress.yml` 已在依赖）。**若你认为这条渠道在本仓库实际上没人看，那么正确做法是删掉这个 workflow 并记录理由**——任务里写明了「不做也是合法结局」，我不希望它变成摆设。**③ 我第一版 T-050 测试不合格，已重写**：只测「重启后能否攻城」看似合格，但变异①②都打不挂它——迁移在重启时清掉旧 `siegeAt`，把两处遮住了。这正是我批评过你的形状（测试通过但守不住名字里写的东西），轮到我自己写一次就犯。现已改为在单次运行内分辨时基。**④ `mailbox[].createdAt` 我复核了你的「仅显示」判断，成立且更彻底**：`public/client.js` 根本没引用过这个字段，是死数据。
- **红线自检**：客户端只提交意图 ✅（T-050 全在服务端判定）｜ 协议未动 ✅（`siegeAt` 不在任何序列化路径上）｜ CHANGELOG 已记玩法改动 ✅
- **下一步建议**：**T-049 与 T-050 都待你复核**。另需你知道：**T-045 已收 Done，但你的独立复核从未发生**——人直接要求收口，我只做了自验（17 处引用核对 + 20 个数字对账 0 不一致）。**若日后那三份文档里发现错误，原因在这里**；你若有余力，仍值得补一次真正的他验。


### 2026-07-19 · Claude → （待指派） · T-049 立项：并发测试覆盖的补位

- **做了什么**：把 T-043 复核时记下的遗留立为 **T-049**（Backlog，**负责人待定**——人本次未指派）。T-043 改串行是必要且已收 Done 的，但默认门禁从此**完全不再并发执行任何测试**，而 T-038 正是「并发满载下的时间依赖」真 bug，`codec.test.js` 的文件级假红也只在并发下现形。
- **改了哪些文件**：`collab/PLAN.md`, `collab/NOTES-claude.md`, `collab/HANDOFF.md`（**无运行时改动**）
- **关联提交**：未提交，见 `collab/review-input.md`
- **验证**：`npm run check` ✅（确认未碰运行时）。立项依据来自 T-043 复核实测：本机 `--test-concurrency=2` **挂死**（两次均 280 秒无输出被终止），串行 **256/256 / 14 秒**。
- **请重点看**：**① 本条明确是保险不是救火，优先级低于任何真 bug**，不要让它挤掉在办任务。**② 建议形状照 `stress.yml` 先例**（定时 / 手动触发 + `timeout-minutes`），**不要挂进 `ci.yml` 的 `server` job**——那会把 T-043 刚移除的假红原样装回去。**③ 并发在受限环境是挂死而非失败，所以 `timeout-minutes` 是硬性要求**，否则是静默烧 CI 时间。**④ 最要紧的一条**：必须说清一次失败如何真正到达人。**没人看的非阻断 job 比没有更糟**，它制造「已有覆盖」的错觉——而错觉正是 T-041 让 main 带红灯上线的病根。**若回答不了「谁会看到、什么时候看到」，正确做法是关掉本条并写明理由**，这比交付一个自我安慰的绿灯诚实。
- **红线自检**：无运行时代码改动 ✅ ｜ 协议未动 N/A
- **下一步建议**：**T-045 仍挂 Review 等 Codex**，是当前唯一在办项；T-049 可在其后按需认领。另有一条 T-048 里我提过、未被回答的建议：盘一遍还有谁在用 `this.time` 做跨重启判断（T-046/047/048 同根）。
