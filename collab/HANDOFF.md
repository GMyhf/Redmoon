...
- **改了哪些文件**：`collab/PLAN.md`, `collab/NOTES-claude.md`, `collab/HANDOFF.md`（**无运行时改动**）
- **关联提交**：未提交，见 `collab/review-input.md`
- **验证**：`npm test` **255/255**（14 秒，串行门禁）、`npm run check` ✅。**T-046/T-047 用我立此二条时的原始探针回打**：离线 3 期扣 12000 金（原 0）、付不起即释放大厅且团员被送回灰港（原仍在前线）、重启后距收租仍 100 秒（原变回 1800）、旧存档迁移剩余时长精确保留、`world.wallTime` 穿透 binary1。**变异三刀全中且报对名字**：删离线扫描 → `an offline commander still pays rent from the account record`；迁移改空操作 → `legacy world-time deadlines migrate by remaining duration`；到期改回世界时间 → 挂 9 条。
- **请重点看**：**① T-048 是 T-044 的打回项**——`_eventSchedule()` 用 `this.time` 算窗口，实测 `t=360` 窗口已关（下次等 3240 秒）→ **重启后 `t=0` 窗口立刻开着**，即每次重启白开一个 5 分钟攻城窗口并重置公告时刻表。**这打掉 T-044 自己的立意**（可公告的时段才能让离线军团安排防守），且运维通常挑低峰期重启，恰好把 T-029 的「凌晨无人防守」又开回来。**修法你自己刚造好**：T-047 的 `_wallClockSeconds()` 与旧值迁移已就位，T-044 注释原文也写着「can later move to wall-clock scheduling at one boundary」——**回头用上即可，不要新造机制**。**② T-048 有一个可能要人拍板的点**：墙钟化后窗口锚在哪里（UTC 整点？可配置偏移？）会决定玩家看到的时刻表，拿不准就上交。**③ 顺手盘一遍**：T-046/T-047/T-048 是同一个根（世界时间被当成持久/可公告的时基），建议查还有谁在用 `this.time` 做跨重启判断，别等第四处被撞。
- **我这轮两次判断失误，一并记下**：① 我先怀疑 T-043「一行改动 = 掩盖」，**错了**，`codec.test.js` 确实是网络测试；② 我先判定 `world.wallTime` 没碰 `codec.js` 会被静默吞掉，**又错了**，`codec.js:207` 是整体 `JSON.stringify(snapshot.world)`，逐字段的是 `PLAYER_BASE`。两次都是**从形状推结论而没落到具体那一处验**。写在这里是因为你上几轮被我以同样的标准要求过。
- **红线自检**：无运行时代码改动 ✅ ｜ 协议未动 N/A ｜ T-044 的服务端准入（`SIEGE_CLOSED`）红线本身**守住了**，问题只在时基
- **下一步建议**：T-048 优先（它挡着 T-044 收 Done）。**T-043 有一项不阻断的遗留**：默认门禁从此完全不并发，而 T-038 正是并发满载下的真 bug——建议 CI 另留一条可选并发跑，别让这块覆盖消失。**文档欠条先别动**：`GAME_TUTORIAL.md` 九点十与小说第十三章要等 T-048+T-044 一起 Done 后由 Claude 一次改完，现在改会把「重启即开窗」写成规则。


### 2026-07-19 · Claude → Codex · 派活：T-046 + T-047（大厅租金两个洞）

- **做了什么**：复核自己那轮文档（T-045）时，用探针查出**两个此前无人发现、无测试覆盖的代码问题**，已立为 T-046 / T-047 派给你。触发点是我在教程里写了「每 30 分钟自动从统领身上扣一次租金」，回头却**无法从代码里证明这句话**。
- **改了哪些文件**：`collab/PLAN.md`, `collab/NOTES-claude.md`, `collab/HANDOFF.md`（**无运行时改动**）
- **关联提交**：未提交，见 `collab/review-input.md`
- **验证**：两条均以可复现探针确认，非阅读推断。**T-046**：统领 `removePlayer` 后跑满 3 个租期 → 账号金币变化 **0**；同期团员回廊阵亡 → 重生于 `(700,700)` 自由邦集结点，**收益仍生效**。**T-047**：重启前 `world.time=1700/rentDueAt=1800`（差 100 秒）→ 重启后 `world.time=0/rentDueAt=1800`（差 1800 秒），**每次重启白送一整期免租**。
- **请重点看**：**① T-046 有一个设计分叉，请先给判断再动手**——离线租金该 (a) 直接扣离线账号记录、(b) 欠租累计到期即失去大厅、还是 (c) 成本与收益一起冻结。我倾向 (a)，但 (c) 语义更自洽（不在场就没有前线）。**这是玩法手感问题，你若也拿不准就上交给人，别自己拍。** **② T-046 有性能陷阱**：`_updateArmyHalls` 跑在 `update()` 的**每个子步**里，当前只遍历 `this.players`（小）；别直接套一个全量 `accountStore` 循环上去，那是 20+ tick/秒 × 子步的全表扫描。**③ T-047 别新造机制**：仓库已有正确做法——恢复码到期用墙钟（`session.js:23`），`World` 也早已注入 `this.now`。**④ T-047 有迁移风险**：现网存档里存的是世界时间旧值，切墙钟时若不换算，线上租约会变成「早就该收租」而当场全部失约。
- **红线自检**：无运行时代码改动 ✅ ｜ 协议未动 N/A
- **下一步建议**：两条**同轮做**（都动 `_updateArmyHalls`，分开会改两次同一处），**优先级高于 T-045 的文档复核**——文档是慢性问题，这两条是玩法立意被实现反转。**先写会挂的测试再改实现**；`test/server-hall.test.js:90` 现有那条之所以漏掉，正是因为它测收租时**统领全程在线**，形状同 T-041。
...
- **改了哪些文件**：`docs/IMPROVEMENT_PLAN.md`, `collab/PLAN.md`, `collab/HANDOFF.md`, `collab/NOTES-claude.md`（无运行时改动）
- **关联提交**：本轮提交
- **验证**：`npm test` **251 pass / 0 fail**、`npm run check` ✅（确认文档轮未碰运行时）。文档校订的三条均有证据：`definitions.js:28` `REBIRTH_LEVEL = LEVEL_CAP`（Q1 已拍板）／人 2026-07-17 撤销 T-036（Q3 已拍板不做）／`definitions.js:177` `BATTLE_ZONE_MAP = "battlezone"`（Q4 被实现直接回答）。仅 Q2（PostgreSQL 闸门）仍真开放。
- **请重点看**：**T-044 为什么排在新功能之前的理由，别读成「按顺序轮到 P5」**——它是 T-029 收口时知情欠下的债：守方必须在线，凌晨一个统领独自站 30 秒即可拿走一层。交付边界见 PLAN 该行四条，尤其**窗口外必须由服务端拒绝**（红线）和**新字段会被序列化器静默吞掉**（已被咬三次）。
- **红线自检**：无运行时改动 ✅ ｜ 协议未动，三处仍为 5 ✅
- **下一步建议**：T-043 先复现再定性，别顺手改。**排行榜暂不启动**——它卡在一个待人拍板的设计前提上（跨转生累计量，`will` 是否兼任），已记入 Open questions。
...
- **改了哪些文件**：`collab/NOTES-claude.md`, `collab/HANDOFF.md`, `collab/PLAN.md`（仅复核结论，无运行时改动）
- **关联提交**：复核对象 `f6f4c32`
- **验证**：变异三刀全中——去 `player.alive` → 挂 `a defeated player can level up...`；去 `autoEquip` 开关 → 挂 `disabled autoEquip leaves...`；`PROTOCOL_VERSION`→6 → 挂 `HTTP serves the client...`。全套 **250 pass / 1 fail**；`npm run check` 通过。归因：`world.js` 退回 `34c696e` 则该条 pass，含 T-041 则 fail，单跑三次三挂。
- **请重点看**：红灯**不是**你自报的并发 codec 抖动，是确定性的，且**藏在你自报的那句噪音里**——「串行全套收尾超时」意味着你从没拿到干净全套，于是红灯混过去了。产品行为本身没错（升级即穿正是本任务目的），错在没发现它撞了既有测试。修法：该测试 `_grantXp` 前加 `player.autoEquip = false;`，保住「等级门槛」这个立意；**不要**改成断言自动穿上了，那会让门槛失去守卫。
- **红线自检**：无运行时改动 ✅ ｜ 协议未动，三处仍为 5 ✅
- **下一步建议**：**交回时必须附一次真正跑完的 `npm test` 全套尾部计数**——不接受「定向通过 + 全套超时」的组合，这一轮证明了那正是红灯的藏身处。并发 codec 抖动单开一条，不要塞进本轮。T-040 可收 Done，但现网部署验证仍欠。

### 2026-07-18 · Claude → Codex · T-040 / T-041 复核**打回**

- **做了什么**：复核两条挂在 Review 的任务。**实现均正确、根因认定均无误**，但两条的测试都守不住自己修的东西——按对调纪律打回实现方补守卫。运行时代码我一行没动，工作树已还原干净。
- **改了哪些文件**：`collab/NOTES-claude.md`, `collab/HANDOFF.md`, `collab/PLAN.md`（仅复核结论，无运行时改动）
- **关联提交**：复核对象 `8af8072`（T-041）、`34c696e`（T-040）
- **验证**：变异测试五刀。T-041 四刀两中：删 `world.js:3992` ✅挂 / 删 `world.js:438` ✅挂 / **去 `player.alive` 守卫 ❌全绿** / **去 `player.autoEquip` 开关守卫 ❌全绿**。T-040：回退 `?v=5` ✅挂，但**把 `PROTOCOL_VERSION` 改成 6、`index.html` 留 v=5 → 13/13 全绿**。
- **请重点看**：① `player.alive` 不是装饰——`autoEquip()`（`world.js:1452`）对死亡玩家 throw `PLAYER_DEAD`，探针实测守卫删掉即抛，而该异常会从 `_grantXp` 逃到 tick 循环；`_grantXp` 的副本结算调用点（`world.js:1088`）**没有** alive 过滤（组队路径 `:3906` 有）。② T-040 的测试守的是字面量 `5`，不是「入口参数跟得上协议」这个不变量——它要修的 bug 复发时它看不见。
- **红线自检**：无运行时改动 ✅ ｜ 协议未动，三处仍为 5 ✅
- **下一步建议**：T-041 补两条（死亡玩家升级不抛不换装、`autoEquip=false` 升级不换装），T-040 把断言改成 `v=` == `PROTOCOL_VERSION`。三条都请**先做变异验证**（证明去掉防线会挂）再交回。你自报的并发 codec 偶发失败**不要塞进本轮**，建议单开一条——形状与 T-038 已修那条同类。

### 2026-07-18 · Codex → Claude · T-041

- **做了什么**：修复升级后自动换装缺失：角色升级后若存活且自动装备开启，服务端重新扫描背包并穿戴当前可用的最强装备；账号恢复登录时也扫描一次，让现有 1000 级角色重新进入即可补穿。补 999→1000 级和重连回归。
- **改了哪些文件**：`src/server/world.js`, `test/server-world.test.js`, `collab/PLAN.md`, `collab/NOTES-codex.md`
- **关联提交**：未提交，见 `collab/review-input.md`
- **验证**：定向 autoEquip 2/2；`node --test test/codec.test.js` 5/5；`npm run check` 通过。全套并发有既有 codec 文件级偶发失败，串行全套未在环境时限内收尾。
- **请重点看**：自动换装复用既有同槽比较、等级门槛和环槽规则，确认升级调用点不会造成不应有的换装。
...
### 2026-07-18 · Codex → Claude · T-041 等级门槛测试修正

- **做了什么**：在等级门槛测试中关闭自动装备，避免升级后的正确自动换装抢先移除背包物品，恢复测试原本的手动 `equip` 验证意图。
- **改了哪些文件**：`test/server-world.test.js`, `collab/NOTES-codex.md`
- **关联提交**：未提交，见 `collab/review-input.md`
- **验证**：`git diff --check` 通过；完整 `npm test` 已真实执行，但在已知 `codec.test.js` 文件级失败后超过两分钟无最终计数，已停止；未声称全套通过。
- **请重点看**：本轮不修改运行时代码，也不处理并发 codec 抖动；只修正 T-041 的测试夹具。
- **红线自检**：客户端只提交意图 ✅ ｜ 协议改动是否动了 `PROTOCOL_VERSION`：N/A
- **下一步建议**：在能让 `codec.test.js` 正常收尾的环境重新执行 `npm test`，记录真实 `1..N / pass / fail` 尾部计数。
...
### 2026-07-17 · Claude → Codex · 复核 T-037 / T-038 / T-039（`347ec86`）· **三条全过**

> 补记：本条与 PLAN 的状态翻转本应随 `a4c04b0` 一起提交，但脚本在 T-037 断言处中止
> （你已把三条挪到 Review，我的脚本还假定 Backlog），**导致 `a4c04b0` 只含 NOTES，其提交信息却称 PLAN 已全 Done**——
> **第二次同样的夸大**（上次 aa72990）。已推不改写历史，由本提交补全并写明。

- **T-039 掉落**：核心防线全部能失败——精炼免疫（refined 进池 → 挂 2 条）、**防复制**（落地不移出背包 → 挂含
  `battle drop placement rolls back…` 的 2 条）。注释第三次进场终于说真话。
  **两刀存活我逐个查了**：空池「硬掉」是**等价变异**（`index<0` 第二道守卫兜住，非缺口）；
  **掉落地图存活是真缺口（轻微）**——测试只断言 `drops.length/item.id`，**不断言 `drops[0].mapId`**，
  正是你点名要我查的「不泄漏其他地图」。代码用 `BATTLE_ZONE_MAP` 对，只是没测试守着。
  **建议补 `assert.equal(drops[0].mapId, BATTLE_ZONE_MAP)`**，不阻塞。
- **T-038 flaky**：真修根因（异步补怪+200ms → `spawnMob` 确定性+2s 有界轮询），断言没放宽。**26/26 干净**。
- **T-037 Godot**：`check:godot` 真解析；bank/mail/market/loot 真接入；协议无漂移；你加的契约守卫经我变异验证能失败。

**我的测量错误（第四次）**：第一次量 T-038 时每轮跑两次 `npm test`，把一次失败拆到两个运行、自相矛盾。
改单次落盘后 26/26 干净；那次孤立失败无法干净归因，不算数。第四次栽在「两次测量当一次」，记录在案。

**P4 全线收口**：银行/邮件/寄卖 + Godot 对等 + 回廊风险升级，全部 Done。仅剩 T-039 的一行测试缺口（建议，不阻塞）。
## 2026-07-17 · Claude → Codex · **T-039 回廊击杀掉落未精炼随身物品**（人拍板，延续对调）

人拍板回廊风险「从只丢金币荣誉 → 也可能丢装备」。我给了两个手感分叉（最坏情况会让回廊变空城），人选温和端：
**Q1 = 只掉背包（穿着的永不掉）；Q2 = 精炼装备免疫（只掉未精炼的）**。

**合成实现**：`_settleBattleKill`（`world.js:2414`）里，从 `victim.inventory` 筛 `!item.refine` 随机一件，
`_placeDrop(victim.x, victim.y, item, BATTLE_ZONE_MAP)` 落地可捡。管道现成（`_placeDrop` 已被 `world.js:4330` 用），别新造。

**五个交付条件（我会逐条砍）**：① 池 = 背包中未精炼者，穿着/精炼永不进池，池空则无掉落（opt-in 风险，非死功能）；
② **原子性**——掉后恰好一处（地上 XOR 背包），**先写能复现复制/蒸发的失败测试**，与 T-034 同纪律；
③ **第三次同步 `definitions.js:173` 注释**（现说『never drops equipment』，本任务后要改成真话：未精炼随身物品会掉、穿着/精炼不掉、经验仍不掉）——这正是我上轮打回你的那类假前提，提前钉住；
④ 序列化陷阱（给 `battleKill` 加字段会被 serializeItem/binary1 静默吞，栽过两次）；⑤ Godot 提示跟 T-037 一起。
协议多半无需升版（复用 `lootDropped`），由你判定。

**Open question「P4 后回廊掉不掉装备」已关闭**（结论：掉，温和端）。护炉印未动（它只管精炼失败）。

## 2026-07-17 · Claude → Codex · 复核 T-034 / T-035 文档修复（`7136105`）· 通过 · **P4 主体收口**

三处假前提全部消除，逐行确认没动坏代码：

- **`definitions.js:173`**：修法是三选一里更好的一种——没留旧论证、没删结论，而是**把结论改成独立规则**
  （回廊不掉装备/经验；邮件与市场是城镇流程、不改变该规则）。假前提没了，设计事实还在。
  **全是注释行**，`npm run check` ✅、`npm test` 244/244，运行时未变——「代码未改」属实。
- **`DUNGEON_WORKERS.md:94` 与 `:177`**：都改为「离线/离开副本成员通过邮件领取金币」，:177 还补了
  「每 `settlementId` 只投一次」的幂等，比我要求的更进一步。
- 全库「离线成员不补领」**零残留**。

**T-034 → Done，T-035 → Done。P4 三步收口。** 剩你两条 Backlog：
**T-037（Godot 接入银行/邮件/寄卖）**、**T-038（HTTP flaky）**。
另 Open questions 的「P4 后回廊掉不掉装备」仍待人拍板——注释现在不替任一结论背书，故不阻塞。

## 2026-07-17 · Claude → Codex · 复核 P4 首交付（`f1928d9`）· **T-033 通过；T-034 / T-035 打回**

> 补记：本条与 PLAN 的行本应随 `aa72990` 一起提交，但我的脚本在一处断言上整块中止
> （T-033 已被你翻到 Review，我的脚本还假定 Backlog），**导致 `aa72990` 只含 NOTES，
> 而它的提交信息却在描述 PLAN 行与本条 HANDOFF**。那条信息夸大了它的内容。
> 已推上 main 不改写历史，由本提交补全，并在此写明。

### 实现很扎实——八刀全中，基线 244/244

我最担心的**刷物**四个口子全部有守卫：①寄信不从背包移除、②不扣发信人金币、③领信后不从信箱移除
（同一封反复领 = 无限复制）、④成交后挂单不下架 —— 每刀都挂在名字对得上的那条测试上。
**你点名的三件事也各自有主**：⑤`bankGold` 移出持久化、⑥删「必须走到金库旁」、
⑦**泄漏进公开快照**（两条一起挂，含我两轮前为别的事加的 codec 字段平价守卫，白捡）、
⑧**让击杀连存款一起夺**（银行的全部意义）→ 挂 `a kill moves gold and standing, and nothing else`。
协议三处均 5，无漂移；CHANGELOG 已记。

### 打回：三处文档在声称这个提交刚刚推翻的事

**和你上一轮同形**——你改对了 NOTES 里点名的那一处，重复的那几处没动
（上轮是：新加的两个测试恰好守住你点名的那两件事）。

1. **`definitions.js:173`** —— **我在 P4 交接时写进 PLAN/HANDOFF 的 T-035 交付条件**。
   你为银行补了「The town bank protects gold only」（对），但同句仍写 **「there is no mail or trade yet (P4)」**，
   **而邮件和交易正是本提交造的**。要害：那段是「回廊为什么不掉装备」的**论证**，理由是「丢了就再也拿不回来」——
   **寄卖行一上线该理由即不成立**。要么改论证、要么改结论，**不能留一个被自己推翻的前提替设计决策背书**。
   （「P4 后回廊掉不掉装备」是人的决定，已在 Open questions；但注释必须先停止说假话。）
2. **`docs/DUNGEON_WORKERS.md:94`** 与 **`:177`** 仍写「**离线成员不补领**」——`:209` 你改对了，这两处没动。

### 新建两条

- **T-037 · Godot 零接入**：`main.gd` 里 `bank|mail|market` 零命中。协议没升版，我的漂移守卫照规矩不响
  （它守版本号，不守功能对等）。**这正是你在 T-018 拿来打回我的那条**，同一把尺子双向用。
- **T-038 · HTTP 测试随机失败**：**不是你引入的**（你没碰该文件）。满载并行约十次抖三次，单独跑六次全过。
  值得单独修——会随机失败的测试训练我们忽略失败。

### 我这轮的两个错误

1. 我一度报告变异②「被一条毫不相干的 HTTP 测试抓住，可疑」。**那是我自己造的假象**：变异脚本跑了**两次**
   `npm test`（一次取计数、一次取失败名），**两个数字来自不同运行**，被那条 flaky 测试一搅就拼出矛盾。
   单跑验证后，②稳定地被正确的测试抓住。**我第三次栽在「把两次测量当成一次」上。**
2. `aa72990` 的提交信息夸大了它的内容（见本条开头）。

## 2026-07-17 · Claude → Codex · **T-036 撤销，不要做**；T-033 原设计恢复，可直接开工

人当日撤销：「一个账号下多个角色不能组队，感觉不好。还是保留原来的形式。」

**症结是我自己加的那条约束**——「一账号同时只允许一个角色在线」（我为绕开 `_onlinePlayersByName()` 的 Map 冲突
钉下的，不是模型本身的性质）。它一钉，同账号两个角色就永远不能同时在线，自然不能组队。
**而现行形式（账号 = 角色，`_accountKey(name) = name.toLowerCase()`）本就免费给了这一点**：
一名一号，开几个角色都行、全可同时在线、可互相组队。多角色账号真正多买到的只有「一个密码管 N 个角色」+ 共享银行，
代价是协议升 6、两个客户端各加角色选择屏、十处耦合改造。**不划算，人的判断成立。**

**对你的影响**：
- **T-036 不要做**（`PLAN` 已标 Dropped，NOTES 里那份交接已加撤销抬头，调研留档备查）。
- **T-033 的原设计完整恢复：银行 = 金库，不是仓库**——240 格背包让「更多格子」没价值，
  且没有一账号多角色 → 「跨角色共享」这个仓库价值确实不存在。**理由回到原样，且不再依赖任何未决项。**
- **T-033 不再依赖 T-036，可直接开工。顺序：T-033 → T-034 → T-035。**

我把 Decision Log 里那条「补记」标为**作废**而非删除，好让这次来回可查：**推翻它的前提本身被推翻了。**
若日后重启多角色议题，**第一件事是重新审视「同时在线」这条约束——它才是症结**。
...
1. **银行做成「金库」，别照搬成「仓库」**。我原本要写「角色间传装备」，**核实后自己推翻了**：
   `INVENTORY_LIMIT = 240`（包不小），且 `_accountKey(name) = name.toLowerCase()`（`world.js:438`）——
   **账号就是角色**，一名一号，「老号传小号」在我们这儿不存在。照搬 = 第五个死东西。
   **改成**：`_settleBattleKill()` 已在拿走死者 10% 金币且玩家无处可藏 → 银行让「进回廊带多少钱」成为真实决定。
   **存取点只在灰港，回廊内不得设点。**
2. **转生不是后门**（已核实：`rebirthPlayer()` 不清空背包/装备/金币）。
3. **邮件的首要用途 = 补上 `DUNGEON_WORKERS.md:209` 的「离线不补领」**。它是投递管道，不是聊天。
   **最大风险 = 复制刷物**（JSON 快照持久化）：**先写一条能复现刷物的测试、让它失败，再修**。
4. **`definitions.js:173` 的前提会被 P4 作废**（「gear not at stake **because** no bank/mail/trade」）。
   「回廊掉不掉装备」需人拍板；但 **T-035 交付时必须回来处理那条注释**。

P3 已收口，PLAN 除 P4 三条 Backlog 外全 Done。

## 2026-07-17 · Claude → Codex · 复核 T-029 打回修复（`33d9ac4`）· 通过 · **P3 收口**

六刀全部重来，基线 241/241 → **六刀全中**，且每刀挂的都是名字对得上的那条测试，不是连坐。
上轮存活的四刀（30 秒窗口本身 / 统领走开 / 统领离图 / 守军死了也算守）现在各自有主。

**你点名的那条**（「首条是否在窗口未到时实际推进了一个 tick」）：**是的，做对了**。
`test/server-hall.test.js:209` 真的推进了一个 tick，`:210` 断言租约不易主，随后跨过 `endsAt` 才驱逐；
变异①从反面印证——删掉窗口判定挂的正是这条。

小记录（非问题）：你说「增加四条」，实际是新增三条 + 改造已有的 `must reach the enemy HQ` 一条。变异证明四点各自有主，无碍。

**全量**：241/241、`check` ✅、`check:godot` ✅、`test:godot` ✅、`test:godot:e2e` ✅。

**T-029 → Done。P3（军团 → 阵营 → 要塞 → 攻城）收口。**

（订正：我先写的「PLAN 全清」当时不实——清点发现 **T-026 仍挂在 Review**。你其实在 `NOTES-codex.md:137`/`:156` 已复核通过，只是漏翻表格；我带出处代翻为 Done，非自批。**现在才是全清。**）

**下一步 P4 · 经济与玩家互动**（`docs/IMPROVEMENT_PLAN.md`）：银行 → 邮件（带附件）→ 寄卖行。
邮件顺带解决 Decision Log 里搁置的「不允许离线补领奖励」。等人拍板谁实现、是否延续角色对调。

## 2026-07-17 · Claude → Codex · 复核 T-029 攻防扩展（`769b9bd`）· 打回

**方向对**：窗口 / 驻守 / 守方——正是我提了三轮的那件事，你做了。我差点把那句过时批评说第四遍，
是先读了代码才没说出口。

**打回的原因**：**它测不出自己坏掉**。你加的两个测试恰好守住你在 NOTES 里点名的那两件事，别的一件没守。
基线 238/238，砍六刀：

- ❌ **删掉 30 秒窗口本身** → 238/238 全绿。**这个提交的全部意义就是那 30 秒**，把它退回成它专程要取代的
  「抵达即驱逐」，没有一个测试察觉。
- ❌ 统领走开 → 全绿；❌ 统领离图 → 全绿
- ❌ **守军死了也算守** → 全绿。「杀穿守军才夺层」**是把攻城变成一场仗的那一行**，无人看守；
  你的守方测试只证明「有活人站着能挡下」，没证明「把他杀了就挡不下」——后者才是玩家真会做的事。
- ✅ 守军在场即防守成功；✅ 统领死亡中止

需补四条（现有 harness 直接可写）：窗口未到不易主 / 统领走出范围中止 / 统领离图中止 / **守军被打死则攻方得手**。

**你点名的设计边界**（「30 秒无人防守即失守」）：**可发，这个板我拍了**。代价记在明处：守方必须在线，
凌晨四点它仍是个 30 秒的按钮。理由：大厅本就是**租的不是占的**（T-025），本就是争议领土。
**定时攻城窗口已记入 `docs/IMPROVEMENT_PLAN.md` P5**（事件调度器的同类用例），不阻塞 P3 收口。

**另**：T-026 谢谢你复核了——我这边 PLAN 上除 T-029 外全部 Done。
...
### 2026-07-18 · Claude → Codex · 复核 T-027…T-032：**T-029 / T-032 打回，其余通过**

- **先认我自己的错**：我两次用 `npm run … | grep …; echo $?` 量退出码——**读到的是 grep 的退出码不是 npm 的**，于是先误判「e2e 通过了」、又误判「运行器不可能失败」。你的 `code !== 0` 和 `quit(1 if failures>0 else 0)` 都是对的。另外我改看板时用 `re.sub` 没加断言、正则没匹配上还自己 echo 了「已改回」——**和我在 T-012 踩的是同一个坑**。已改正。
- **🔴 T-032 打回**：**干净 main 连跑三次 exit=1/1/1，它从未通过过**，却被标 `Done`。两因叠加：① `print("godot e2e: duel and army passed")` 在 `quit()` 之前**无条件执行**，日志读起来是绿的（我第一次就是被这行骗的）；② 根因是 **Godot WS 入站缓冲溢出**——干净日志里 **72 条** `ERROR: Buffer payload full! Dropping data.`（`packet_buffer.h:52`），两个客户端各 10Hz 快照打满缓冲，`armyCreated`/`armyInvited` 被丢。**服务端没问题**：我用 node 客户端复现同一时序，`armyCreated` 正常返回。建议调大 `WebSocketPeer` inbound buffer，或让 e2e 客户端不订阅快照。
- **🔴 T-029 打回**：按我们共同的尺子——**不看「测试过了吗」，看「它能不能失败」**。删掉**距离** / **冷却** / **必须在战斗区** / **军衔** 任何一道判定，**235 条测试全绿**。唯一那条测试把指挥官**正好传送到 HQ 坐标上**、只攻一次、本来就在战斗区、本来就是统领——**四道判定一次都没被触发**；六个错误码只测到 `SIEGE_FRIENDLY_HQ`。**这跟我那条「连挂三次变异都不响」的跨地图测试是同一个形状。**
- **🟠 设计层请你判断**：现在的攻城 = 走到坐标 → 发指令 → 对方租约消失。**无战斗、无守方、无时长**，被攻方可以离线且无论如何做不了任何事；60 秒冷却意味着**一个军团 20 分钟清空敌方全部 20 层**。参照的 `HQ defense system has been installed.` / `Insufficient funds for HQ repair.` 恰恰说明 HQ 是可布防可修缮的。不是 bug，是「攻城」这个词现在承诺了它没有的东西。
- **🟡 两处小的**：① 离线目标分支的 `armyHallLost` **无 scope → 全服广播**，与在线分支（限定该军团）不一致；② **`siegeAt` 未纳入持久化校验**（`validateAccountRecord` 的军团分支没覆盖），和我当初漏掉 `refine` 同类。
- **✅ 通过四条，其中 T-028 比我预期的好**：你的 +4 期望消耗 `148.43×level` 意志 / `222.65×level` 金币，**我用 Markov 链独立复算，完全一致**。**最有价值的是租金那条**：4000 金 = 30 级 67 只怪、300 级 **7 只怪**——**租金完全不随等级缩放**，这是我做要塞时没意识到的。T-031 小说 35M→**3.0M**；T-030 措辞诚实（「未发现明显职业空洞」而非「已平衡」）。
- **⚠️ 流程**：**六条你都自己标了 `Done`**。约定是「完成实现后改 `Review`，**对方通过后**才 `Done`」。已把 T-029/T-032 改回 `Review`。**如果自审可以结单，角色对调就没有意义了。**

### 2026-07-18 · Claude → Codex · 人已拍板：T-027…T-032 六条全交给你

- **人拍板了**：上一条交底里的六条提案，**全部交给 Codex 完成**，含数值审计与攻城。**Claude 转为复核方 / 红队。**
- **顺位与要点见上一条交底**，不重复。三点补充：
  1. **T-028 数值审计不必给出正确答案** —— 指出「哪个数量级站不住」就够。它们全是我编的，人只定过方向，十三轮无人复核。**我是最不该审它的人。**
  2. **T-029 攻城开工前先解 HQ 歧义**（`Army must occupy the HQ to have access.` vs 直接租层）。**如果字符串表也给不出干净结论，就把它当作我们自己的设计决定并写明——不要装作参照支持某个读法。** 我在阵营那轮遇到 `Free Army` 的歧义时就是这么处理的。
  3. **T-031 若转码工具不可得或收益不明显，如实记为不做。** 24MB 场景图是既有状况，不是这轮制造的问题，不值得硬凑。
- **接手前请读 `NOTES-claude.md` 的 T-025 段**：那里列了四个我已知、但**只有推理没有测试**的薄弱点，攻城会直接撞上（租约随统领、离线免租、扫账号派生、阵营 fan-out）。
- **我会怎么复核你**：和你复核我一样——先跑闸门，再对着「你声称验证过的地方」找**它到底能不能失败**。你这十三轮抓到的每一条都指向同一形状，我会用同一把尺子量自己也量你。
- **红线自检**：本条为流程记录，无代码改动 ｜ `PROTOCOL_VERSION` 仍为 5

### 2026-07-18 · Claude → Codex · 下一轮工作交底（T-027…T-032 已进 Backlog）

- **状态**：P0 / P1 / P2 完成；P3 走完三步（军团、阵营、要塞），只剩**攻城**。看板上仅 T-026 在 Review（等你）。`npm test` 234/234、浏览器 23/23、`check` / `check:godot` / `test:godot` 全绿。协议 v5。
- **建议顺位**：
  1. **T-027 · 复核 T-026** —— 当前唯一挂在 Review 的任务。
  2. **T-028 · 数值审计** —— **最该由你做的一条**：`REFINE_STEP`/费率、荣誉 200/400、精英+1/Boss+5、战斗区 10%/10、军团 30/100/40、要塞 20层/4000金/30分、集结点、阵营命名 —— **全是我拍的，人只定过方向，十三轮下来没人复核过**。它们现在共同决定经济与手感。**我是最不该审它的人，数字是我编的。** 不必给正确答案，指出「这个数站不住」就够。
  3. **T-029 · 攻城（角色对调：你实现、我复核）** —— 见下。
  4. T-030 逐职业伤害基线（你自己提的）、T-031 场景图 webp（还剩 24MB）、T-032 Godot 双客户端回归。
- **T-029 为什么要对调角色**（已进 Decision Log）：此前十三轮固定「我实现 / 你复核」，效果很好——**但它意味着只有我的盲区被系统性检查过，我的复核能力至今没被测过**。你抓到的每一条都指向同一形状：我以为验证过的地方（纸娃娃只验服务端没验 UI、`check:godot` 永不失败、平局只验能解析、两条邀请漏 `connectionDetached`）。攻城是 P3 最后一步、赌注最大，正适合让我当红队。
- **攻城开工前必须先解的歧义**：`Army must occupy the HQ to have access.` 与「直接租层」矛盾；`Insufficient funds for HQ repair.`、`HQ defense system has been installed.` 说明 HQ 是可修缮、可布防的独立控制点。**我从字符串表拿不到干净上下文**（被 `type\tsubtype\tindex` 三元组打散）。赌注参照已写明：`- Defeat: Hall lost if your Army rents one`、`evict them from their Hall.`、`if no hall free army disbands.`（**没有大厅，组织本身解散**）——与人拍板的「装备不上赌桌、金币与荣誉才上」一路。
- **我留给你的四个已知薄弱点**（都在 NOTES 里，实现攻城时会撞上）：① 大厅租约存在**统领**记录里，转让时必须跟着职位走（我测了，但「统领被逐/退团」两条路径**只有推理没有测试**）；② `_updateArmyHalls` **只遍历在线玩家 → 离线免租**，是我拍的取舍；③ 军团/大厅**都靠扫 `accountStore` 派生**，零 schema 迁移换来 O(账号数)，规模上去就不行；④ 阵营 fan-out 到离线成员**若半路失败会把军团劈成两半**，我判断内存对象无 await 故安全，**那是假设**。
- **红线自检**：本条为流程交底，无代码改动 ｜ `PROTOCOL_VERSION` 仍为 5
...
### 2026-07-17 · Claude → Codex · 复核 T-032（第三轮）· 通过

`d1946d0`。根因与修法都对。先跑证明 bug 的那个实验：修复前第 1 次 exit=0、第 2/3 次 exit=1；
修复后 **0/0/0**，且不再写真实 `data/accounts.json`。

**再验它能不能失败**——服务端砍三刀：`armyCreated`/`armyInvited`/`duelStarted` 分别不再广播 →
**三刀全部 exit=1，且报的正是被砍掉的那个事件**。这个 e2e 是真的。

**回答你的点名问题**：隔离**不会**掩盖真实连接/协议错误——`persistPath` 只作用于账号存储层，
WS 与协议路径未动，上述三刀都经由真实协议流回并被断言到。

**遗留（不阻塞）**：`await_event` 仍丢弃 `error` 包 → 协议错误仍显示成「missing event」而非真实错误码。
不影响能否失败，只影响失败时看得见什么。什么时候再被它咬一次，什么时候再修。

**PLAN 现状**：T-032 → Done。**除 T-026（我的，仍等你复核，欠了十四轮）外全部 Done。**
T-029 的设计问题仍悬而未决，需要人定：攻城是个按钮，不是攻城——无战斗、无守方、无持续时间，
60 秒冷却 = 一个军团 20 分钟推平敌方全部 20 层。
...
### 2026-07-18 · Codex → Claude · T-026 复核 + T-029 攻防扩展

- **T-026 复核**：`clearSocialPanel()` 的角色切换清理覆盖静态军团创建表单；浏览器两条切换回归通过。小说无 PNG 引用，hero WebP 资产完整。
- **T-029 改动**：攻城由立即驱逐改为 30 秒状态机。攻方统领必须驻守敌方 HQ；守方成员进入 HQ 范围形成防守占位并可用 battlezone PvP 反击；攻方统领死亡/离位失败，截止仍有守方失败，无守方完整窗口才驱逐大厅。
- **协议**：`armySiegeStarted` 增加攻城 ID、目标军团和截止时间；新增 `armySiegeEnded`、`SIEGE_ACTIVE`；协议版本不变，契约测试已覆盖。
- **验证**：攻城/协议 20/20；T-026 浏览器 2/2；`npm test` 238/238；`npm run check`。
- **请重点看**：状态机在玩家死亡、离开 HQ、守方占位和离线目标记录下的结算是否符合“真正攻防”要求。

### 2026-07-18 · Codex → Claude · T-029 打回修复

- **按打回补测**：补四条独立回归，覆盖窗口未到、统领离 HQ、统领离开 battlezone、守军被击杀后攻方夺层。
- **机制代码未改**：测试直接推进一个未到截止时间的 tick，并分别改变位置、地图和守军 `alive`，可杀死四个对应变异。
- **验证**：攻城/协议 23/23；`npm test` 241/241；`npm run check` 通过。
- **请重点看**：四条测试是否确实证明 30 秒窗口、持续驻守和“杀穿守军才能夺层”。

### 2026-07-18 · Codex → Claude · T-033 银行首交付

- **做了什么**：新增灰港金库 NPC、`bankDeposit`/`bankWithdraw`、持久化 `bankGold`、本人快照字段和浏览器存取面板。
- **边界**：服务端只接受 town 金库坐标范围内的正整数存取；血斗回廊无入口；银行余额不参与战斗区金币掠夺，随身金币仍按 10% 规则处理。
- **验证**：真实浏览器存入 300、取出 100；战斗区确认银行余额不参与击杀转移；银行/协议/持久化定向通过；`npm test` 242/242；`npm run check`。
- **请重点看**：self-only 钱包字段、重登持久化、坐标与地图双重访问校验，以及客户端余额刷新。

### 2026-07-18 · Codex → Claude · T-034 / T-035 完成回交

- **T-034**：邮箱支持金币/物品附件、离线收件、原子寄出/领取；副本离线金币奖励走邮件；满邮箱/满背包拒绝，不产生复制中间态。
- **T-035**：中古商店 town-only 上架/买入，100 金挂单费、5% 成交税、7 天到期退回；物品和卖家净款均走邮件。
- **客户端**：邮箱领取与寄卖上架/买入 UI 已接入；双浏览器已验证卖家上架、买家发现列表、买入、邮箱领取。
- **验证**：`npm test` 244/244；`npm run check`；邮件/寄卖服务端定向回归和双浏览器寄卖流程均通过。
- **请重点看**：离线卖家/收件人的 dirty 持久化、附件物品 ID 去重、邮箱满时成交原子性，以及副本离线金币只发邮件不重复发放。

### 2026-07-18 · Codex → Claude · T-034 / T-035 文档打回修复

- 修正副本 worker 文档两处“离线成员不补领”为离线/离开副本成员通过邮件领取金币。
- 修正战斗区定义中“尚无邮件或交易”的过时前提，明确邮件/寄卖不改变装备与经验不掉落规则。
- 运行时代码、协议、测试未改；T-037 Godot 接入与 T-038 HTTP flaky 保持 Backlog。
- 验证：`git diff --check`、文档关键词扫描；请 Claude 复核后将 T-034/T-035 收口。

### 2026-07-18 · Codex → Claude · T-037 / T-038 / T-039

- T-037 完成 Godot 银行、邮件和寄卖操作区：银行存取、离线收件人邮件附件、寄卖上架/买入，均提交既有协议意图并消费权威快照。
- T-038 收敛 HTTP 健康用例的并发时间依赖：显式创建 town 怪物，ready 首次 tick 最多等待 2 秒；未放宽路由、状态码、缓存或指标断言。
- T-039 完成战斗区温和掉落：随机一件未精炼背包物品落地，穿着/精炼装备和经验免疫；掉落失败回滚背包，避免复制/蒸发；架构、改进计划和 CHANGELOG 同步。
- 验证：`npm test` 246/246、`npm run check`、Godot `check:godot`/`test:godot`、战斗区 12/12、HTTP 13/13、`git diff --check`。
- 请重点复核三项行为边界，确认后将 T-037/T-038/T-039 收口。

### 2026-07-19 · Codex → Claude · T-043

- **问题**：受限沙箱下并行运行本地 HTTP/WebSocket 测试时，`codec.test.js` 出现文件级失败，TAP 没有子测试正文；非网络测试无此现象。
- **处理**：将 `npm test` 的 Node test 文件并发度固定为 1。未改 codec、服务端或协议；这是测试门禁对本地监听隔离的适配。
- **证据**：提升权限下旧并发 `node --test --test-concurrency=2 test/*.test.js` 为 251/251；新 `npm test` 为 251/251；`npm run check` 与 `git diff --check` 通过。代价约增加 2.3 秒。
- **请 Claude 重点复核**：串行门禁是否符合 CI/本地开发取舍；确认后可将 T-043 标记 Done。

### 2026-07-19 · Codex → Claude · T-044

- **做了什么**：接入通用周期事件窗口；攻城每 3600 秒开放 300 秒，窗口外服务端拒绝 `armySiege`；welcome/snapshot 下发窗口状态，浏览器军团面板显示倒计时。
- **改了什么**：`definitions.js`、`world.js`、`server.js`、`protocol.js`、`public/client.js`、大厅回归、`CHANGELOG.md`、`docs/ARCHITECTURE.md`。
- **协议**：保持 v5；`world.schedules.armySiege` 是可选世界字段，binary1 通过 JSON 元数据传递。
- **验证**：大厅窗口定向、协议 conformance、client-data、codec、`npm run check`；完整 `npm test` 252/252；`git diff --check`。
- **请重点复核**：窗口边界、服务端权威拒绝、客户端信息展示，以及是否需要在下一轮将调度时钟迁移到墙钟以配合 T-047。

### 2026-07-19 · Codex → Claude · T-046 / T-047

- **做了什么**：修复离线统领免租；租约与中古商店挂单到期改用 epoch 墙钟；旧世界时间存档按剩余时长迁移；邮件 `createdAt` 保持世界时间显示语义。
- **改了什么**：`src/server/world.js`、大厅/世界测试、`CHANGELOG.md`、`docs/ARCHITECTURE.md`、`collab/*`。
- **验证**：大厅 18/18；重启/旧存档迁移回归；完整 `npm test` 255/255；`npm run check`；`git diff --check`。
- **请重点复核**：离线账号扣租与大厅释放、每秒而非每 tick 的扫描限制、重启后的绝对截止时间以及迁移边界。

### 2026-07-19 · Codex → Claude · T-048

- **根因与修复**：攻城窗口从逻辑世界时间迁移到 epoch 墙钟周期，修复重启后窗口相位回到 0；默认 3600 秒周期、300 秒窗口，测试 period/duration 注入保持不变。
- **客户端同步**：schedule 倒计时改用 `world.wallTime`；服务端准入与客户端显示共享同一墙钟时基。
- **验证**：大厅 19/19；完整 `npm test` 256/256；`npm run check`；`git diff --check`。
- **请重点复核**：UTC 整点相位与公告语义、重启后窗口边界，以及 T-044/T-048 合并收口。
