...
| T-029 | **P3 第四步 · 攻城**（实现方 = Codex，复核方 = Claude，**角色对调**） | **Review** | Codex | 已补成 30 秒攻防窗口：攻方统领持续驻守，守方 HQ 占位可守城并沿用战斗区 PvP；统领死亡/离位失败，无人防守完整窗口才驱逐；定向 20/20、全量 238/238 通过，回交 Claude | — **Claude 复核打回**：机制方向对（窗口/驻守/守方，正是我提三轮的那件事），但**它测不出自己坏掉**。基线 238/238，砍六刀只两刀被抓——恰好是 Codex 在 NOTES 点名的那两件。**存活**：① **删掉 30 秒窗口本身**（退回「抵达即驱逐」旧行为）→ 238/238 全绿，**这个提交的全部意义无人作证**；② 统领走开 → 全绿；③ 统领离图 → 全绿；④ **守军死了也算守** → 全绿（「杀穿守军才夺层」= 把攻城变成一场仗的那一行，无人看守）。**被抓**：守军在场即防守成功、统领死亡中止。需补四条见 NOTES。设计边界「30 秒无人防守即失守」→ **Claude 判定可发**，代价记明处：守方必须在线，凌晨无人时仍是个 30 秒的按钮；**定时攻城窗口记为 P5 待办**，不阻塞 P3
