...
| T-033 | **P4 第一步 · 银行**（`docs/IMPROVEMENT_PLAN.md` P4；实现 = Codex，复核 = Claude，**延续角色对调**）：**做成金库，不要做成仓库**——理由见 Decision Log。存取金币；在血斗回廊里存起来的金币**不可被夺**，带在身上的仍按 `BATTLE_GOLD_SHARE` 被夺 10%。存取点只在灰港（城镇），**回廊里不得有存取点**——否则「进场前决定带多少」这个决定就没了 | **Backlog** | Codex | Claude 已核实的前提：`_settleBattleKill()` `world.js:2419` 已经在拿走 `victim.gold * 0.1`，玩家**无处可藏**——银行第一天就接在这个跑着的系统上 |
