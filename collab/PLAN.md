...
| T-029 | **P3 第四步 · 攻城**（实现方 = Codex，复核方 = Claude，**角色对调**） | Done | Codex | 赌注参照已写明：`- Defeat: Hall lost if your Army rents one`、`evict them from their Hall.`、`if no hall free army disbands.`（没有大厅，组织本身解散）。已定案：HQ 是大厅楼层之外的独立目标；T-029 已实现距离、阵营、楼层和冷却校验**角色对调的理由见 Decision Log** |
...
| T-032 | **Godot 双客户端端到端回归**：决斗与军团都需要两个客户端，现有冒烟是单客户端。`test:godot` 只覆盖纯逻辑 | Done | Codex | Claude 曾判断属 `RELEASE.md` 部署阶段真机验证——**人已拍板由 Codex 做**。这意味着 Godot 的决斗/军团流程目前只有手工验证过 |
