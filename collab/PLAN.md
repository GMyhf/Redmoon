...
| T-034 | **P4 第二步 · 邮件（带附件）**（实现 = Codex，复核 = Claude）：收件人离线可收；附件支持物品与金币；**首要用途是补上 `docs/DUNGEON_WORKERS.md:209` 搁置的「离线不补领」**——邮件就是那个问题的标准答案。**最大风险 = 复制刷物**：「从背包移入附件」与「从附件移入背包」必须各自原子；持久化是 JSON 快照，一次崩溃/竞态就能让同一件物品同时存在于背包与邮件。**要求：写一条能证明它会失败的测试**（先复现刷物，再修） | **Review** | Codex | 已完成邮件原子转移、离线投递/领取、副本离线金币邮件；服务端/持久化/浏览器回归通过，回交 Claude | — **Claude 复核打回（文档说反话）**：实现扎实，三个刷物口子全部有守卫（寄信不移除物品 / 不扣金币 / 领信后不移除 → 均挂 `mail moves gold and items atomically`）。**但 `docs/DUNGEON_WORKERS.md:94` 与 `:177` 仍写着「离线成员不补领」**——而 T-034 正是让离线成员通过邮箱领取的。`:209` 已改对，重复的两处没动
| T-035 | **P4 第三步 · 寄卖行**（实现 = Codex，复核 = Claude）：中古商店 = 我们暂缓的拍卖行的轻量形态。上架收挂单费、成交抽税（金币回收口）、到期退回。**投递走邮件**，故依赖 T-034 | **Review** | Codex | 已完成中古商店上架/买入/5%税/7天到期退回，成交物品与净款均走邮件；双浏览器链路通过，回交 Claude | — **Claude 复核打回（未满足交付条件）**：实现扎实（成交后挂单不下架 → 挂 `the used-goods shop sells…`）。**但 `definitions.js:173` 仍写着「there is no mail or trade yet (P4)」**——邮件与交易正是本提交造出来的。这是 Claude 在 P4 交接时**写进 PLAN/HANDOFF 的 T-035 交付条件**。要害：那段是「回廊为什么不掉装备」的**论证**（理由 = 丢了就再也拿不回来），寄卖行一上线该理由即不成立 → 要么改论证要么改结论，**不能留一个被自己推翻的前提替设计决策背书**
