...
| T-036 | **一账号多角色**（人 2026-07-17 指示「改成类似红月的」；实现 = Codex，复核 = Claude）：**排在 T-033 之前**——它把 T-033 的前提改回来了。参照的模型（Claude 已查证 `common.inc.php:63`：`SELECT * FROM tblGameID1 WHERE BillID = ?`）：**`tblBillID.BillID` = 账号（登录名/密码/`is_hardcore`/`LastLogin`）**，**`tblGameid1.GameID` = 角色**（等级/脸型/声望/奖金点数）靠 `BillID` 外键挂在账号下，角色名全局唯一。**迁移可无损**：现有每条记录（键 = 角色名）→ 一个账号（handle = 该名）下挂一个同名角色。**切分建议（最小差异）**：只有 `tokenHash`/`recovery` 上移到账号层，其余 `ACCOUNT_FIELDS` 全部留在角色层（含 `gold`——它在血斗回廊里是要被夺走的，必须随身而非随账号）；**银行（T-033）成为唯一跨角色共享的东西**，这恰好就是参照里 warehouse 的位置。**一账号同时只允许一个角色在线**（否则 `_onlinePlayersByName()` 的 Map 会静默吞掉第二个）。**协议升 6**：`join` 就是登录，「选哪个角色」现在无处可问，需新增前置意图 + 两个客户端的角色选择屏 | **Backlog** | Codex | Claude 已勘定的改动面（agent 全量扫描）：**① `_accountKey`（`world.js:438`）是服务端唯一的身份原语**，同时是持久化主键 / 凭证主体 / 审计主体 / 跨玩家查找键；**② 规则有三份拷贝**——`world.js:438`、`public/client.js:510-515`、Godot `main.gd` 的 token 存储，三处必须同时改；**③ `_armyRoster`（`world.js:1636`）是铰链**——它拿 `accountKey` 既当去重键又当显示名兜底，故 `army.memberName`（今为可选，`server.js:1878`）必须转为必填；**④ `kickArmy`/`promoteArmy`（`world.js:2128`/`:2162`）直接拿打进来的角色名索引账号库**，需要新建 名字→(账号,角色) 索引；**⑤ `_securityCheckpoint`（`server.js:1204`）假定一条凭证命令只碰一条记录**；**⑥ `archetype` 锁**（`world.js:342`「One name is one character, forever」）语义要重述为「一个**角色名**一个职业」；**⑦ `session.js` 零身份耦合**，是唯一无需改的一层；**⑧ 存储可无 DDL**：`crimson_accounts.record` 是 jsonb，记录内嵌 `characters: {}` 即可，但 `schema_version` 须越过 2、`validateAccountRecord`（`server.js:1830`）须递归；**⑨ `highestItemSequence`（`world.js:154`）须多下潜一层**，否则物品 id 会重号 |
| T-033 | **P4 第一步 · 银行**（`docs/IMPROVEMENT_PLAN.md` P4；实现 = Codex，复核 = Claude，**延续角色对调**）：**依赖 T-036，须排其后**。原「做成金库不做成仓库」的理由**已被 T-036 推翻一半**（见 Decision Log 2026-07-17 补记）：一账号多角色一旦成立，「跨角色共享」这个仓库的经典价值就回来了。**最终形态 = 金库 + 仓库**：存起来的金币在回廊不可被夺（金库），且**银行是唯一跨角色共享的东西**（仓库）。存取金币；在血斗回廊里存起来的金币**不可被夺**，带在身上的仍按 `BATTLE_GOLD_SHARE` 被夺 10%。存取点只在灰港（城镇），**回廊里不得有存取点**——否则「进场前决定带多少」这个决定就没了 | **Backlog** | Codex | Claude 已核实的前提：`_settleBattleKill()` `world.js:2419` 已经在拿走 `victim.gold * 0.1`，玩家**无处可藏**——银行第一天就接在这个跑着的系统上 |
| T-034 | **P4 第二步 · 邮件（带附件）**（实现 = Codex，复核 = Claude）：收件人离线可收；附件支持物品与金币；**首要用途是补上 `docs/DUNGEON_WORKERS.md:209` 搁置的「离线不补领」**——邮件就是那个问题的标准答案。**最大风险 = 复制刷物**：「从背包移入附件」与「从附件移入背包」必须各自原子；持久化是 JSON 快照，一次崩溃/竞态就能让同一件物品同时存在于背包与邮件。**要求：写一条能证明它会失败的测试**（先复现刷物，再修） | **Backlog** | Codex | 依赖 T-033 |
| T-035 | **P4 第三步 · 寄卖行**（实现 = Codex，复核 = Claude）：中古商店 = 我们暂缓的拍卖行的轻量形态。上架收挂单费、成交抽税（金币回收口）、到期退回。**投递走邮件**，故依赖 T-034 | **Backlog** | Codex | 依赖 T-034 |
...
> **2026-07-17 补记（人指示后订正）**：人已决定**做成一账号多角色**（T-036）。
> 上面第 2 条理由（「没有一账号多角色」）因此**即将不成立**——我不能让这条决策继续声称一个要被推翻的前提。
> **订正后的结论**：银行 = **金库 + 仓库**。第 1 条理由（240 格背包 → 「更多格子」没价值）**依然成立**，
> 所以仓库的价值**不是格子数，是跨角色共享**；金库的价值（进回廊带多少钱）不受影响，两者叠加。
> **切分因此定为**：`gold` 留在角色层（它在回廊里要被夺走，必须随身），**银行是唯一跨角色共享的东西**——
> 这恰好就是参照里 warehouse 的位置。**T-033 须排在 T-036 之后。**
