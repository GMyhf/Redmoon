...
| T-004 | **Phase 6 · 副本 worker 硬化**：I1 异步 tick 链背压、跨 worker 故障/epoch 端到端回归、跨机调度演练、协议 conformance、容量/压力门 | In progress | Codex | **I1 背压过审（`c7b159f`）**：每副本单 in-flight + 合并 `pendingDt`，与 `world.update` 的 0.5s 防穿透 clamp 安全组合，背压测试扎实。小注：clamp 丢弃的滞后未进 backlog 指标。**剩余**：跨 worker 故障/epoch 端到端回归（建议下一块）、协议 conformance、容量/压力门、T-002 去抖 |
