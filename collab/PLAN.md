...
| T-003 | **副本 worker 集成**：把 worker 接进 `world.js.enterDungeon`（起 worker、路由输入、回投 tickResult 快照/事件给成员、`settle` 经 `settleDungeon` 幂等发奖、退役 3a 进程内路径） | Review | Codex | **集成过审（`ec1ee82`）**；I2 事件字段已修，I3 worker→完成→settle 端到端测试已补。🟠 I1（异步 tick 链无背压）归 Phase 6 |
