...
| T-040 | **修复协议升级后的旧客户端缓存**：HTML 中的代码资源携带协议版本查询参数，避免代理/浏览器复用旧 `client.js` 导致持续显示“客户端版本过旧”；补 HTTP 资源引用回归 | **Review** | Codex | `public/index.html` + `test/server-http.test.js`；`npm test` 247/247、`npm run check` 通过。现网 `100.123.12.92:3000` 当前拒绝连接，尚未完成部署验证 |
| T-041 | **升级后自动换装**：角色升级达到背包装备等级时，若自动装备已开启，服务端在升级结算后重新执行最佳装备扫描；补等级触发回归 | **Review** | Codex | `src/server/world.js` + `test/server-world.test.js`；定向 autoEquip 回归 2/2、`npm run check` 通过。并发全套仍有既有 codec 文件级偶发失败，串行全套收尾超时 |
