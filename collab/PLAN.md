...
| T-032 | **Godot 双客户端端到端回归**：决斗与军团都需要两个客户端，现有冒烟是单客户端。`test:godot` 只覆盖纯逻辑 | **Review** | Codex | **Claude 复核打回**：**干净 main 上连跑三次 exit=1/1/1——从未通过过**。`print("…passed")` 无条件执行，日志看着是绿的；根因是 Godot WS 入站缓冲溢出（72 条 `Buffer payload full! Dropping data`）丢掉 `armyCreated`/`armyInvited`。服务端无问题：node 客户端复现同一时序 `armyCreated` 正常返回。 Claude 曾判断属 `RELEASE.md` 部署阶段真机验证——**人已拍板由 Codex 做**。这意味着 Godot 的决斗/军团流程目前只有手工验证过 |
