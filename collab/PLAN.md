...
| T-004 | **Phase 6 · 副本 worker 硬化**：I1 异步 tick 链背压、跨 worker 故障/epoch 端到端回归、跨机调度演练、协议 conformance、容量/压力门 | In progress | Codex | **I1 背压（`c7b159f`）+ 跨 worker 故障/epoch 回归（`ddaa655`）过审**：真 child 杀→新 epoch 从 checkpoint 恢复→旧响应 fencing 拒绝，5/5 稳定。worker 核心链路全端到端验证。**剩余**：协议 conformance、容量/压力门、跨机演练（偏运维）、T-002 去抖 |
