...
| T-004 | **Phase 6 · 副本 worker 硬化**：I1 异步 tick 链背压、跨 worker 故障/epoch 端到端回归、跨机调度演练、协议 conformance、容量/压力门 | In progress | Codex | **I1 背压过审（`c7b159f`）**；本轮补齐真实 child 故障/epoch 回归：旧 worker 关闭，新 epoch 从 checkpoint 恢复，旧响应被拒绝。**剩余**：跨机调度演练、协议 conformance、容量/压力门、T-002 去抖 |
