...
| T-004 | **Phase 6 · 副本 worker 硬化**：I1 异步 tick 链背压、跨 worker 故障/epoch 端到端回归、跨机调度演练、协议 conformance、容量/压力门 | In progress | Codex | **Codex 已认领 I1 背压**：每副本单 in-flight tick + 合并 `pendingDt`，避免 IPC 慢时 Promise 链无界增长；其余 Phase 6 项留后续 |
