...
2. **副本独立化**：当前确定性副本仍运行在主进程内。后续迁移到带版本票据的独立 worker，让玩家状态和当前默认 5 分钟断线保席支持跨 worker 续接。
3. **Godot 发布验证**：协议接入、主要玩法、账号/队伍/副本功能和无头烟测已完成；继续补 macOS/Linux 打包签名、输入法、断网边界和跨平台发布验证，不再优先做纯视觉装饰。
