...
| `upgrade` | `skill` | 消耗技能点 |
| `respawn` | 无 | 请求合法重生 |
| `rebirth` | 无 | 达到 1000 级满级后转生：等级归一，换取永久属性点、生命与伤害加成 |
...
| `refine` | `item`, `useProtection?` | 在锻匠·坤铁附近精炼稀有及以上装备（背包内或已穿戴均可）。消耗意志与金币，服务端摇点：0→4 阶成功率依次 90%/70%/50%/30%，失败掉 1 阶；`useProtection` 为真时消耗 1 张护炉印保住当前阶数。**推到 +3/+4 还需 200/400 荣誉——荣誉是门槛，只被检查、不被扣除** |
| `partyInvite`/`partyAccept`/`partyLeave` | `target` / `from` / 无 | 组队邀请、接受与离开（上限 4 人，附近队友共享 60% 经验） |
| （无指令） | — | **战斗区 `battlezone`（血斗回廊）**：走城镇门环的第十道门即进入，**全服唯一一张任何人都能攻击任何人的地图**。投射物只在该地图上对**同一地图**的其他玩家碰撞（`mapId` 比较是隔离的全部依据）。玩家击杀结算：击杀者取走 10% 金币与**至多对方实际拥有的**荣誉（上限 10），并可能使死者背包中随机一件未精炼物品掉在原地；**穿着的装备、已精炼装备与经验不损失**。小号无荣誉可夺、互喂为零和；邮件和寄卖行不改变这条掉落规则。被怪物杀死不额外结算 |
...
| `armySiege` | `camp` + `floor` | **攻城**（统领限定）：HQ 是大厅楼层之外的独立目标。攻城还必须处于服务端 UTC epoch 周期公告时段（每 3600 秒开放 300 秒，跨重启保持相位）；窗口外返回 `SIEGE_CLOSED`。统领必须在血斗回廊抵达敌方 HQ 的服务端坐标范围内，且通过敌我阵营、楼层和 60 秒军团冷却校验；开始后进入 30 秒攻城窗口。攻方统领必须持续驻守 HQ，守方成员进入 HQ 范围即形成防守占位，并可使用战斗区 PvP 击杀攻方；统领死亡/离位或窗口结束时仍有守方则攻城失败，只有无人防守的完整窗口才驱逐指定楼层租约。 |
...
| `chat` | `channel`, `text` | 聊天：`global`（全服，含大厅）/`map`（本图）/`party`（组队）/`army`（军团，只到达在线团员，无军团时返回 `NO_ARMY`）。文本去控制字符、截 200 字符，0.6 秒冷却（`CHAT_TOO_FAST`）；无队伍用组队频道返回 `NO_PARTY` |
...
九张环境概念图以 WebP 保存在 `public/assets/scenes/*.webp`，用于美术参考；原始 PNG 仍保留作源资产。实际地图背景仍使用 `public/assets/textures/*.webp` 材质和 Canvas 地形渲染，不把概念图作为固定背景加载。
...
当前欢迎消息携带 `protocol: 5`。v2 引入账号会话令牌（`join.token`、`session` 消息与 `NAME_IN_USE`/`INVALID_TOKEN` 错误）；**v3 引入装备精炼**：物品记录新增 `refine`（0-4，为 0 时字段缺省，JSON 与 binary1 两种编码一致），玩家自身记录新增 `protections`（护炉印计数），新增 `refine` 指令与 `itemRefined` 事件，binary1 的公开装备段每件追加一个 `u8 refine`。**v4 把 `honor` 从自身记录移到每个玩家的公开记录**（binary1 公开玩家段新增一个 i32）——荣誉的分档本来就是给别人看的，战斗区是它第一次有社会意义的地方。**v5 给每个玩家的公开记录加上 `armyCamp`**（binary1 公开段新增字符串）——血斗回廊里阵营决定谁能被攻击，必须在开火之前就可读。发生破坏性字段变化时必须递增协议版本；进入滚动升级阶段后至少兼容相邻版本。
...
当前已提供第一个确定性组队副本、容量门、超时、一次性奖励和 15 秒断线保席。副本进入后由
`GameServer` 启动受监督的 child worker；主进程负责认证、席位、输入路由、snapshot/event 回投
和 reward-once 账本，worker 只推进副本实体。worker 失联时主进程使用检查点续接或失败回收，
避免重复角色、奖励或物品。下一步是跨机实例调度与真实故障恢复演练。

### 3. Godot 原生客户端

版本化 schema（`src/server/protocol.js`）与协议一致性测试已完成。Godot 客户端位于
`clients/godot`：除等距渲染、二进制快照和输入外，已经覆盖属性/技能升级、商店、银行、
邮件、寄卖、队伍、自动化、转生、恢复/轮换和组队副本，并有真实服务器无头烟测。下一步优先做 macOS/Linux
打包签名、输入法和断网边界，不继续投入纯视觉装饰。浏览器继续作为完整试玩入口；二进制
快照 `binary1` 实测约为 JSON 体积的 1/3，后续再按容量证据决定是否引入增量快照。

### 4. 长线玩法

**PvP 与拍卖行已不再暂缓**：决斗场、荣誉与独立战斗地图（`BATTLE_ZONE_MAP`）PvP 已交付，
拍卖行的轻量形态「中古商店寄卖」已随 P4 经济交付。**仍暂缓**：更多地图、赛季、匹配评分与反串通审计——
账号安全、真实 PostgreSQL 运维、容量观测和副本 worker 路由稳定后再评估。
