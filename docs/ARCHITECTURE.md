...
当前欢迎消息携带 `protocol: 3`。v2 引入账号会话令牌（`join.token`、`session` 消息与 `NAME_IN_USE`/`INVALID_TOKEN` 错误）；**v3 引入装备精炼**：物品记录新增 `refine`（0-4，为 0 时字段缺省，JSON 与 binary1 两种编码一致），玩家自身记录新增 `protections`（护炉印计数），新增 `refine` 指令与 `itemRefined` 事件，binary1 的公开装备段每件追加一个 `u8 refine`。发生破坏性字段变化时必须递增协议版本；进入滚动升级阶段后至少兼容相邻版本。
