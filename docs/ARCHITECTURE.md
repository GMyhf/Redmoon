...
当前实现是单进程、单区域；战斗世界状态在内存中，账号进度通过 JSON 文件持久化。
它适合玩法原型和局域网测试，尚不承诺水平扩展、恶意客户端防护或无缝升级。
...
当前使用原子替换的 JSON 存档保存账户、角色、背包和成长；文件为带版本的信封结构
`{ "schema": 1, "savedAt": …, "accounts": { … } }`，旧的无版本平铺格式加载时自动迁移，
遇到比当前版本更新的 schema 拒绝启动而不是悄悄改写。存档含会话令牌，落盘时文件权限
收紧为 0600、目录 0700。生产 systemd 状态目录为
`/var/lib/crimson-relay`。后续可引入 PostgreSQL 保存账户、角色、背包、成长和审计日志。
世界 tick 不直接等待数据库；通过命令/事件边界异步持久化，并以幂等事务保护奖励发放。
认证、角色服务和实时世界分离后，再增加短期会话令牌。
