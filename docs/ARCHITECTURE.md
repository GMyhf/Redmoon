...
积压达到 2 MiB 或连续跳过 50 个可丢弃帧时断开。离散事件仍尽量发送，但同样受 2 MiB
硬上限保护。阈值通过 `WS_HEARTBEAT_INTERVAL_MS`、`WS_BACKPRESSURE_SKIP_BYTES`、
`WS_BACKPRESSURE_DISCONNECT_BYTES` 和 `WS_BACKPRESSURE_MAX_SKIPS` 调整。

非主动断线默认保留原玩家席位 15 秒（`WS_RECONNECT_GRACE_MS`，设为 `0` 可关闭）：玩家暂停
...
- `/health` 是纯存活探针，始终以 200 返回世界、持久化和运行时诊断。
- `/ready` 是流量就绪探针；持久化失败、tick 超过 1 秒未成功、连续 3 次世界循环失败、
  event-loop lag p99 超过 250 ms、快照广播 p99 超过 250 ms，或任一 WebSocket 积压达到
  256 KiB 时返回 503。响应的 `checks` 给出实际值和阈值。
- `/metrics`（及 `/api/metrics`）输出 Prometheus 文本，覆盖 tick age/错误、lag、快照耗时、
  WebSocket 当前积压、跳帧和心跳/背压断开计数；生产代理不向公网暴露它。
