...
| （无指令） | — | **战斗区 `battlezone`（血斗回廊）**：走城镇门环的第十道门即进入，**全服唯一一张任何人都能攻击任何人的地图**。投射物只在该地图上对**同一地图**的其他玩家碰撞（`mapId` 比较是隔离的全部依据）。玩家击杀结算：击杀者取走 10% 金币与**至多对方实际拥有的**荣誉（上限 10）——小号无荣誉可夺、互喂为零和。**装备与经验一律不损失**（无银行/邮件/交易兜底前不放上赌桌）。被怪物杀死不额外结算 |
...
| `armySiege` | `camp` + `floor` | **攻城**（统领限定）：HQ 是大厅楼层之外的独立目标。统领必须在血斗回廊抵达敌方 HQ 的服务端坐标范围内，且通过敌我阵营、楼层和 60 秒军团冷却校验；成功后只驱逐指定楼层租约，目标军团仍可重新租赁。 |
