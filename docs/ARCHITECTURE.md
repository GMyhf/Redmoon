...
版本化 schema（`src/server/protocol.js`）与协议一致性测试已完成；最小 Godot 客户端位于 `clients/godot`（连接/入场/快照渲染/输入/离开，含会话令牌与无头烟测模式），下一步充实 Godot macOS/Linux 客户端。浏览器仍是快速试玩入口。高频快照成为瓶颈后，可在不改变玩法语义的前提下引入二进制编码、增量快照、插值与客户端预测。
