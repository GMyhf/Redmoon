...
| （无指令） | — | **战斗区 `battlezone`（血斗回廊）**：走城镇门环的第十道门即进入，**全服唯一一张任何人都能攻击任何人的地图**。投射物只在该地图上对**同一地图**的其他玩家碰撞（`mapId` 比较是隔离的全部依据）。玩家击杀结算：击杀者取走 10% 金币与**至多对方实际拥有的**荣誉（上限 10）——小号无荣誉可夺、互喂为零和。**装备与经验一律不损失**（邮件和寄卖行目前只提供找回/投递通道，不改变掉落规则）。被怪物杀死不额外结算 |
